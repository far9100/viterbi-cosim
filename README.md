...
`make report` 把這條紀律機械化：**190 條 assertion，每一條都同時驗
...
| 條件 | 實測 | 裁決 |
|---|---|---|
| **F1** d\* < 1 m 或不存在？ | 所有模型/環境/η 下的最小 d\* = **17.8 m** | 不觸發 → 存活 |
| **F2** 兩模型下 \|Δd\*\| 皆 < 5%？ | 模型 A **+11.29%**（自由空間）/ **+6.31%**（室內） | 不觸發 → 存活 |
| **F3** 符號：A 正、B 負？ | A **正**、B **負**（−0.75% / −0.43%） | 不觸發 |
...
`make report` 跑 `scripts/check_paper_numbers.py`：**190 條 assertion，mismatches: 0**。
...
涵蓋 `docs/report.md` 與本 README，外加 **26 個 gate** 的完整性檢查。
（連「190 條」這句話本身都被驗——README 說幾條就必須真的有幾條。這條會在每次新增 assertion 時
