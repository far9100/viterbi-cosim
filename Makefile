...
# ---------------------------------------------------------------- 單元測試
#
# C2′ 也納入。它是 CPU 與 torch 兩條實作的零容忍位元級比對（47 條），
# 先前只有 `make m2` 走得到 —— 於是開發時跑 `make test` 看到全綠，
# 卻完全沒有碰到整個掃描層的正確性，而 M2 的 BER 全部是那一層算出來的。
# CPU 那一輪不需要 GPU（`e7284f5` 已解除硬相依），沒有 CUDA 時它會如實回報跳過幾條。
test:
	@$(ENV) $(PY) -m pytest tests/ sweep/test_c2prime.py -q
