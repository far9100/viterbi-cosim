...
**原文（§2）**：「**每個位元的有效回溯深度都恰好是 D。** 這是「traceback depth D」的教科書定義，
也是讓 D 軸有意義的唯一寫法——D=24（低於 5K=35）會可觀地變差，**D=64 接近最佳**。」

**更正**：前半（D=24 會可觀地變差）成立；**後半「D=64 接近最佳」撤回**。
`data/d_sweep.csv` 的 windowed(D) − ML 損失為
D=24: **+0.209** dB、D=32: **−0.050** dB、D=48: **+0.058** dB、D=64: **−0.072** dB。
這三個較深的點不但**非單調**，而且出現**負損失**——窗口回溯贏過全幀 ML，數學上不可能——
顯示 D=32/48/64 全部落在量測雜訊地板（**±0.076** dB）之內。
因此只可宣稱「D 低於 5K = **35** 會可觀地變差」，**不可宣稱 D=64 優於 D=32**。

這條勘誤不影響本文件定義的回溯語意（uniform depth D），只影響對 D 軸結果的解讀。

**證據**：`data/d_sweep.csv` 的 `loss_vs_ml_db` 欄。

**發現於**：CHANGELOG `2026-07-29-07`。
