...
| Gate | 已知答案 | 驗證對象 | 容差 |
|---|---|---|---|
| E1–E3 | 環境（見 §9 M0） | 工具鏈 | 已於 M0 全綠 |
| G1 | 未編碼 BPSK，BER = 10⁻⁵ 需 Eb/N0 = 9.588 dB | L1/L2 通道模型與 AWGN scaling | ±0.1 dB |
| **G2a** | 實測 K=7 soft BER **≤ (133,171) 的 union bound** | 解碼器演算法 | **零容忍**（這是可證明的上界，不是拍腦袋的容差） |
| **G2b** | 未量化 + D=64 參考組態下，@1e-5 對未編碼的增益 | 解碼器演算法 | **[5.0, 5.6] dB** |
| G3 | 3-bit 軟判決 vs 無限精度，**在掃描網格中最佳 clip 下**損失 ≈ 0.2 dB | 量化器設計 | ±0.15 dB |
| G4 | 硬判決 vs 軟判決，損失 ≈ 2 dB | Branch metric | ±0.3 dB |
| G5 = C2 | L2 vs L3 位元級相等：每 stage 比對 `bm[4]`、`pm[64]`、`survivor[64]`、**解碼位元** | RTL 正確性 | **零容忍** |
| G6 | Modulo **決策等價**（見下） | 字寬選擇 | **零容忍**（安全格點）；4 個不安全格點須觸發 |
| **G7** | 雙模擬器一致性（Verilator 2-state vs Icarus 4-state） | reset 完整性 | **零容忍** |
| **C2′** | L2-CPU vs L2-GPU 位元級相等 | GPU golden model | **零容忍** |
