...
>
>    **`surv` 的 R² = 0.000** —— 倖存者決策與 SNR 的相關性是**零**。
>    而 PM + min-PM 只佔 ~7–13% 的功耗，所以 pm 那 3.3% 稀釋到總功耗只剩 ~1%。
>
>    **注意用詞**：pm 在 2 dB 有 0.3% 的凹陷，**不是嚴格單調**。
>    5 個點的「看起來單調」不是證據，R² 才是。本文件先前版本寫成「單調上升」，
>    已依實測更正。
>
