...
**原文（§1 表格）**：B2 的「記憶體」欄寫 **64 × 2D**。

**更正**：那是**演算法**需要的歷史長度，不是**硬體**需要的緩衝深度。
golden 保留完整歷史，所以這個差別在 `mode='batch'` 上看不出來；
硬體是環狀緩衝，而**回溯進行中寫入仍在繼續**：

* 批次在 `t_end` 觸發，引擎每個 stage 走 2 步、共跑 D 個 stage；
* 期間又有 D 筆新的 survivor 寫進來（`t_end+1 .. t_end+D`）；
* 而引擎最舊要讀到 `t_end-2D+1`。

從「最舊仍需要的項目」到「最新寫入的項目」橫跨 **3D**，所以環狀緩衝至少要 3D 個 slot。
實測（`scripts/verify_batch_memory.py`）：2D 在 k = 2D−1 那一步被覆蓋，
**3D 無衝突**。

**對事前預測的影響**：§5 的 **P-B3**（`u_tb` 面積比 B1′ 增加 +60% 以上）
是以「記憶體深度 D → 2D」推出來的，前提偏低。
**但預測原文不改、原樣裁決** —— 事後調整預測會讓預先登記失去意義
（比照規格書 §6 對 G4b 的處理）。裁決時如實記載「前提偏低」這件事即可。

**證據**：`scripts/verify_batch_memory.py`。

**發現於**：CHANGELOG `2026-08-01-14`。
