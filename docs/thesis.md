...
### 6.8 低功耗基準線:在最佳化過的設計上,那個 null 還在不在(M9)

<!-- 來源: report.md §7, lowpower_baseline.md, data/results_m9_*.csv -->

§6.6 的負面結果有一個當時未被檢驗的前提:**它是在一個未最佳化的設計上量到的。**
一個沒有 clock gating、每個 stage 改寫全部 64 個 survivor 暫存器的設計,
本來就很難看出功耗隨資料改變。判準與事前預測凍結於
`docs/lowpower_baseline.md`(量測前提交),定義三態 B0(原 RTL)/ B0′(改寫 RTL,
控制混淆因子)/ B1′(改寫 RTL + clock gating),三者皆先過閘級 C2 才准量功耗。

**兩條事前預測被推翻。** P1 登記「clock gating 的總功耗降幅 < 10%」,
實測 **−42.7%**(44.143 → 25.275 mW @3 dB);推理錯在機制——省下的不是閒置週期的
時脈,而是每個 flop 的回授 mux。分區塊:traceback **−58.5%**、ACS −14.5%、
min-PM +0.1%(純組合邏輯,沒有 flop 可 gate),traceback 佔比自 66.75% 降至 48.4%。
P3 登記「面積可能上升(多了 ICG cell)」,實測面積**下降**:
B0 157,527 → B0′ 163,888(**+4.04%,純 RTL 改寫的代價**)→ B1′ 140,166 µm²
(−11.02% vs B0、**−14.47% vs B0′**)。沒有 B0′ 這一欄,就會把 −11.02% 全歸給
clock gating。

**null 的裁決是「未解析」,而且判準本身需要先補一塊。** 依凍結判準原樣套用:
B1′ 全距 1.47%(< 2%,過關)但 R² 0.568(≥ 0.5,不過關),落在判準表第三列。
同一份文件要求先以 ≥ 8 個獨立 seed 建立 null 分布,補上之後 σ_null = **0.1415 mW**、
全距 **0.941%**,而跨 SNR 的全距是 **0.914%**——兩者幾乎一樣大。
據此追加斜率檢定(**事後追加,如實標示**,不用它替換凍結判準):
t = **+1.82**,95% 區間跨過零。這正是 §6.6 的措辭必須從「效應不存在」改為
「效應被侷限在累加器、僅佔約 1% 總功耗,本量測解析不出」的理由。
佐證判準的侷限:同一個設計、同一組激勵,B0 的 R² 是 0.478、B0′ 是 0.551——
光是改寫 reset 寫法就跨過了 0.5 門檻,5 個點的 R² 解析不出趨勢。

**最強的一條:起伏的形狀是激勵的指紋。** 兩條互相獨立的證據——
(a) 跨 SNR 的變異等於純 seed 的變異,且在總功耗砍掉 42.7% 之後依然成立
(B0′ 比值 0.971、B1′ 1.035);(b) 三態的正規化起伏形狀幾乎完全相同
(r = +0.9925 / +0.9898 / +0.9997)。三個設計裡有一個功耗少 42.7%、RTL 寫法不同,
它們唯一的共同點是每個 SNR 點用同一個激勵 seed。若起伏來自 SNR,
它應隨資料路徑改變;若來自「抽到哪組資料」,就會像這樣被三個設計一起複製。

**M9 沒有涵蓋的**(不得默示為已驗證):B2(memory traceback)從未建置,
因此 `lowpower_baseline.md` 的 **P4 與 P5 至今未裁決**;M9 只動合成選項與 RTL 寫法,
沒有動架構。另見 §7.3 關於 M9 預先登記的時間戳只支持 60 秒這一項揭露。
...
7. **M9 的「預先登記」沒有時間戳證據。** 本文主張預先登記時,依據的是 commit 時間戳:
   登記文件必須在它所管轄的量測開跑之前入庫,而時間戳可由 git 驗證。M5 那一對站得住——
   ...
   **M9 那一對站不住**:`docs/lowpower_baseline.md` 與 `data/power_m9.json` 的 commit
   只相隔 **60 秒**,而且 M8/M9 的四個 commit 全部落在 82 秒內、與 CHANGELOG 記載的
   工作日期不一致——那批工作是後來一次補 commit 的。60 秒**分不出**
   「先寫登記文件、再跑量測」與「量完之後補寫文件、兩者一起 commit」,
   而後者正是預先登記唯一要排除的情形。
   M9 的事前預測**確實寫在一份獨立文件裡且被自己推翻**
   (P1 登記 < 10%、實測 −42.7%——事後補寫不會這樣寫),
   但支持它的是文件內容與 CHANGELOG 的敘述,**不是 commit 時間戳**;
   讀者不應把 M9 的預先登記與 M5 的等同視之。
   `scripts/check_paper_numbers.py` 對此設了 1.0 小時的最小間隔門檻,
   低於門檻的配對預設紅燈,只有在逐字揭露實際間隔後才放行——這段揭露是機械化強制的。
...
- **折疊架構的 PPA(已開工,量測未完成):** 原裁定跳過(2026-07-17),
  2026-08-01 經作者書面確認後補做。判準與五條事前預測凍結於
  `docs/folded_architecture.md`(量測前提交),其中把
  「折疊架構可零修改沿用 testbench」這句從未被驗證過的宣稱降級為兩條可證偽的預測。
  RTL 與量測尚未執行。
- **memory traceback(B2)對照(已開工,RTL 未完成):** 判準與事前預測凍結於
  `docs/memory_traceback_baseline.md`(量測前提交);golden 端的 `mode='batch'`
  已完成並通過凍結的交叉檢查(不一致率 0.0166%,門檻 1%)。RTL 未做——
  實作前發現 B2 會改動 frame 層的輸出時序、不是模組層的 drop-in,
  且環狀緩衝需要 3D 而非文件原寫的 2D(勘誤 E-06),設計推導見
  `docs/m14_implementation_notes.md`。
- **SRAM macro survivor memory:** 以 `sky130_sram_1rw1r_64x256_8` 取代 flop 陣列,
  可把 d\* 從上界收緊為實測。**仍未開工。**
- **post-route P&R 與真實 Fmax:** 補上繞線寄生、clock tree 與 setup 最佳化,
  給出可信的最高頻率。**仍未開工。**
